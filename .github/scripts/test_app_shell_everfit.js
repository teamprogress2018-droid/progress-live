...
ok('library flyout items', /Ćwiczenia/.test(html) && /Plany \/ treningi/.test(html) && /Generator AI/.test(html));
...
ok('flyout css', css.includes('.nav-flyout') && css.includes('.cl-everfit-row'));
ok('flyout js', /function\s+toggleLibraryFlyout/.test(nine) && /function\s+closeLibraryFlyout/.test(nine));
ok('goTo marks library group', core.includes("libraryScreens") && core.includes('nav-library-btn'));
